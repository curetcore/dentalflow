...
interface SectionDividerProps {
  type?: 'wave' | 'wave-opacity' | 'curve' | 'triangle' | 'tilt' | 'arrow' | 'book' | 'wave-smooth'
  color?: string
  flip?: boolean
  className?: string
}
...
export default function SectionDivider({ 
  ...
  return (
    <div 
      className={`absolute ${flip ? 'top-0' : 'bottom-0'} left-0 w-full overflow-hidden leading-[0] ${flip ? 'rotate-180' : ''} ${className}`}
    >
      <svg 
        className="relative block w-[calc(100%+1.3px)] h-[30px] md:h-[50px] lg:h-[80px]"
        data-name="Layer 1" 
